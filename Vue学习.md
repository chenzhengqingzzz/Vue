...
`this.$router.push({})`：内传的对象与`<router-link>`中的to相同，跳转到指定hash地址，并增加一条历史记录

`this.$router.replace({})`：跳转到指定的hash地址，并替换掉当前的历史记录
...
它代表着在匹配这个路由的时候作进行的操作，你可以在路由配置上直接定义beforeEnter守卫：
